...
const TableHead = forwardRef<HTMLTableCellElement, HTMLAttributes<HTMLTableCellElement>>(
  ({ className, ...props }, ref) => (
    <th
      ref={ref}
      className={cn('h-12 px-6 text-left align-middle font-semibold text-xs uppercase tracking-wider text-gray-600 bg-gray-50 [&:has([role=checkbox])]:pr-0', className)}
      {...props}
    />
  )
)
...
const TableCell = forwardRef<HTMLTableCellElement, HTMLAttributes<HTMLTableCellElement>>(
  ({ className, ...props }, ref) => (
    <td ref={ref} className={cn('h-16 px-6 align-middle [&:has([role=checkbox])]:pr-0', className)} {...props} />
  )
)
